...
画像は、画像比率とcanvas比率からWebGLのUVを`contain`方式で補正する。横長画面ではX軸、縦長画面ではY軸のUV範囲を狭め、画像全体の縦横比を維持する。トリミング設定がある場合は切り出し後の実効比率を再計算して表示領域へ拡大する。画像と画面の比率が異なる部分は黒い余白として表示する。
...
全モードは同じ画像テクスチャ、顔追跡、指ドラッグを共有する。通常は5つのプリセットから1つを選ぶ。`複数処理を組み合わせる`を有効にすると、ホログラム、ポップ3D、前景微振動、Depth多層を独立に選び、Depth多層／微振動の座標変位、Depth照明、ホログラム／浮彫り材質、共通輪郭補正の順に1本のshaderパイプラインで合成する。効果強度は材質反射または浮き彫り効果へ適用し、既存の立体強度は視差移動量として独立させる。
...
- 設定表示中だけ、WebGLとは別の透明canvasへトリミング枠、キャラ中心／範囲、疑似光源から中心へ向かうRayを描く。
- 人物自動分離または任意マスク読込後は、マスクを薄い水色で重ね、非ゼロ画素の外接矩形を破線で表示する。これは認識結果の確認用であり、マスク自体を変更しない。
- 左／右／上／下トリミングは元画像UVの参照範囲だけを変更する。`輪郭ぼかし`はトリミング境界のフェザー幅としてshaderへ渡す。
- Undo／Redoは画像やAIマップを複製せず、エフェクト、照明、中心、範囲、トリミング等の編集値を最大60状態保持する。初期値リセットも履歴へ積まれるためUndo可能である。
